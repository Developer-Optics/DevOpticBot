...
class Exp(Cog):
	...
	async def check_lvl_rewards(self, message, lvl):
		if lvl >= 50: # T5
			if (new_role := message.guild.get_role(801762057401008149)) not in message.author.roles:
				await message.author.add_roles(new_role)
				
		elif 40 <= lvl < 50: # T4
			if (new_role := message.guild.get_role(801761930673913857)) not in message.author.roles:
				await message.author.add_roles(new_role)

		elif 30 <= lvl < 40: # T3
			if (new_role := message.guild.get_role(801761827183656960)) not in message.author.roles:
				await message.author.add_roles(new_role)

		elif 20 <= lvl < 30: # T2
			if (new_role := message.guild.get_role(801761678270005298)) not in message.author.roles:
				await message.author.add_roles(new_role)

		elif 10 <= lvl < 20: # T1
			if (new_role := message.guild.get_role(801760823348690944)) not in message.author.roles:
				await message.author.add_roles(new_role)
	...
	async def on_ready(self):
		if not self.bot.ready:
			self.levelup_channel = self.bot.get_channel(790624630666035271)
